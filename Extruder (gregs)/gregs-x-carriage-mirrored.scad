// MODIFIED VERSION
// X-carriage mirrored (two LM8UUs on the motor side)
// GNU GPLv2
// Modified from Greg Frost's x-carriage
...
mirrored_carriage=true; // Set to true to have the side with the belt have two lm8uu's
endstop_kicker=true; // Set to true to add a small "kicker" for the endstop
...
echo ("belt_clamp_width",belt_clamp_width);
...
//			translate([-25-1, 0, 0]) cylinder(r=2, h=3, center=true);

			// End stop kicker
			if (endstop_kicker) {
				translate([-10, holder_separation/2+lm8uu_holder_length-9, 0])
				difference() {
					cube([30, 9, lm8uu_support_thickness]);
					translate([0, 9, 0])
					rotate([0, 0, 45])
					cube([4, 4, 2*lm8uu_support_thickness], center=true);
				}
			}

			// =======================================================
			// Belt clamp socket hangouts
			// =======================================================
			if (mirrored_carriage) {
				for(i=[-1,1])
				translate([21.5, i*(holder_separation/2+lm8uu_holder_length+belt_clamp_width/2)])
				translate([0, -belt_clamp_width/2])
				difference() {
					cube([20, belt_clamp_width, lm8uu_support_thickness]);
					translate([0, belt_clamp_width/2+i*belt_clamp_width/2, 0])
					rotate([0, 0, 45])
					#cube([belt_clamp_width/2-.7, belt_clamp_width/2-.7, 2*lm8uu_support_thickness], center=true);
				}

				for (i=[-1,1])
				translate([25+13.5+1,i*(holder_separation/2+lm8uu_holder_length+belt_clamp_width/2)])
				rotate(90*(i+1)+180) 
				belt_clamp_socket ();
			} else {
				for (i=[-1,1])
		...
		// Leave these holes for normal belt clamp in case someone wants to mount something on that side
		for (i=[-1,1])
		translate([-25-13.5-1,i*(lm8uu_holder_length/2+belt_clamp_width/2)])
		rotate(90*(i+1)+180) 
		#belt_clamp_holes();
